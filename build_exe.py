#!/usr/bin/env python3
"""
Скрипт для сборки исполняемого файла OzonParserAPI.exe с помощью PyInstaller
"""
...
def create_spec_file(main_file):
    ...
    # Собираем дополнительные файлы и папки
    data_files = []
    additional_items = [
        'config',
        'logs', 
    ...
    print("🔍 Поиск дополнительных файлов и папок...")
    for item in additional_items:
        if os.path.exists(item):
            if os.path.isdir(item):
                data_files.append(f"('{item}', '{item}')")
                print(f"  ✅ {item}/ (папка)")
            else:
                data_files.append(f"('{item}', '.')")
                print(f"  ✅ {item}")
        else:
...
exe = EXE(
    ...
    upx_exclude=[],
    runtime_tmpdir=None,
    console=False,  # Всегда без консоли для GUI приложения
...
def simple_build_exe(main_file):
    ...
    try:
        # По умолчанию без консоли (GUI режим)
        console_mode = False
        ...
        data_items = [
            ...
            ('config.txt', '.'),  # Добавляем config.txt рядом с .exe
            ('requirements.txt', '.')
        ]
        ...
        hidden_imports = [
            ...
            'selenium', 'fastapi', 'uvicorn', 'uvicorn.main', 'uvicorn.server', 'uvicorn.config',
            'pydantic', 'pydantic_settings', 'pyngrok', 'pyngrok.ngrok', 'aiohttp', 'requests'
        ]
        ...
        cmd.extend(['--collect-all', 'pyngrok'])  # Включаем все файлы pyngrok
...
def build_exe():
    ...
    print("🚀 Начинаем сборку OzonParserAPI.exe...")
    print("📱 Режим: GUI приложение без консоли")
    ...
    # По умолчанию используем простую сборку (без .spec файла)
    use_spec = False
...
def check_exe_file():
    ...
    exe_path = Path('dist/OzonParserAPI.exe')
    if exe_path.exists():
        ...
        print("🌐 Приложение включает GUI и API сервер")
        
        # Автоматически показываем путь к файлу
...
if __name__ == "__main__":
    print("=" * 60)
    print("🔧 Сборка OzonParserAPI.exe")
    print("=" * 60)
    print("📋 Этот скрипт создаст исполняемый файл, который включает:")
    print("   • GUI интерфейс для управления API")
    print("   • FastAPI сервер с ngrok интеграцией")
    print("   • Selenium парсер с undetected-chromedriver")
    print("   • Все необходимые зависимости")
    print("=" * 60)
    ...
    if success:
        ...
        print("   1. Запустите OzonParserAPI.exe")
        print("   2. Откроется GUI с вкладками управления")
        print("   3. API автоматически запустится с ngrok туннелем")
        print("   4. Используйте публичный URL для интеграции с Google Sheets")
        print("   5. Если возникают проблемы, проверьте логи в папке logs/")
    else:
        ...
        print("   4. Использовать простую сборку вместо .spec файла")
