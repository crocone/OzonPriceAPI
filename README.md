...
- **Google Sheets интеграция** - парсинг артикулов прямо из таблиц
- **Батчевая обработка** - автоматическое сохранение после каждого батча
- **Парсинг выделенных артикулов** - возможность парсить только выбранные товары
...
- `PROXY_LIST_PATH` - путь до файла со списком прокси

### Проксирование

1. Создайте файл со списком прокси в формате `host:port:login:password` (по умолчанию `config/proxies.txt`).
2. Оставляйте пустые строки или строки, начинающиеся с `#`, для комментариев — они будут проигнорированы.
3. При каждом запуске драйвера прокси выбирается случайным образом из списка. Если файл отсутствует или пуст, запросы пойдут напрямую.
...
Проект включает Google Apps Script для работы с таблицами:

### Новые функции:
- **Парсить все артикулы** - обрабатывает все артикулы в таблице
- **Парсить выделенные артикулы** - обрабатывает только выделенные ячейки с артикулами
- **Автоматическое сохранение** - данные сохраняются после каждого батча запросов
- **Прогресс-индикатор** - показывает текущий прогресс обработки

### Установка в Google Sheets:
1. Откройте Google Sheets
2. Перейдите в Extensions → Apps Script
3. Скопируйте файлы из папки `ozon_parser/` в редактор скриптов
4. Сохраните проект
5. Обновите страницу с таблицей
6. В меню появится "🟣 OZON Парсер"

### Использование:
1. **Настройте API URL** - укажите ваш ngrok URL
2. **Подготовьте таблицу** - добавьте колонки для артикулов и результатов
3. **Парсите артикулы**:
   - Все артикулы: Меню → Парсить все артикулы
   - Выделенные: Выделите ячейки → Меню → Парсить выделенные артикулы

### Структура таблицы:
| Артикул | Наименование | Продавец | CPP | Цена без Ozon | Первоначальная цена | Доступность |
|---------|--------------|----------|-----|---------------|-------------------|-------------|
| 158761892 | ... | ... | ... | ... | ... | ... |
